...
class OLE
{
    const OLE_Psb8_TYPE_ROOT = 5;
    const OLE_Psb8_TYPE_DIR = 1;
    const OLE_Psb8_TYPE_FILE = 2;
    const OLE_DATA_SIZE_SMALL = 0x1000;
    const OLE_LONG_INT_SIZE = 4;
    const OLE_Psb8_SIZE = 0x80;
    ...
    public function _readPpsWks($blockId)
    {
        ...
        foreach ($this->_list as $pps) {
            if ($pps->Type == self::OLE_Psb8_TYPE_DIR || $pps->Type == self::OLE_Psb8_TYPE_ROOT) {
                $nos = [$pps->DirPps];
                $pps->children = [];
                while ($nos) {
                    $no = array_pop($nos);
    ...
    public function isFile($index)
    {
        if (isset($this->_list[$index])) {
            return $this->_list[$index]->Type == self::OLE_Psb8_TYPE_FILE;
        }
    ...
    {
        if (isset($this->_list[$index])) {
            return $this->_list[$index]->Type == self::OLE_Psb8_TYPE_ROOT;
        }
